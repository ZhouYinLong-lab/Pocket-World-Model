...
The v2 route-aware protocol removes that hidden 32-step cap, uses learned collision-response prediction for multi-step position scoring, and adds an RGB-only four-action route follower with landing safety checks. Across 20 episodes per map and seeds 11/23/41, the 64-step two-waypoint result reaches **3.94 ± 0.27px train / 4.13 ± 0.30px unseen 20-step position error**, **97.8% ± 3.1pp / 93.3% ± 2.4pp task success**, and **98.3% / 93.3% waypoint completion**. The 96-step three-waypoint stress test reaches **96.7% / 93.3% task success** and **97.4% / 96.1% waypoint completion**. These are hybrid planner results: the route controller uses only visible RGB geometry and recent RGB velocity, while the learned model supplies collision-aware prediction and the learned open-space planner remains available as the fallback for wall-free scenes. See [two-waypoint result](results/evaluation-map-suite-v2-3seed-collision-guard.json) and [three-waypoint result](results/evaluation-map-suite-v2-3seed-waypoint3.json).

Collision frequency is not yet uniformly mature: the two-waypoint protocol averages about **1.16 collisions per train leg and 2.41 per unseen leg**, concentrated in a few cross/double-barrier seed/map pairs. The maturity-gate report adds 50-episode-per-seed calibration and shift detection: in-distribution 90% position/velocity coverage is **91.8%/91.9%**, changed-map coverage is **87.5%/90.8%**, and changed-map AUROC is about **0.95**. Fast-speed velocity coverage and joint map+speed coverage remain below the desired 85–95% band. See [maturity result](results/evaluation-maturity-v2-3seed.json).
