...
The environment now has named map suites rather than only one implicit wall layout. `train` contains `default`, `single_barrier`, and `double_barrier`; `holdout` contains the unseen `cross` and `zigzag` layouts. The new evaluator reports per-map RGB prediction error and a sequential two-waypoint task that must complete two ordered goals. The first three-episode diagnostic against the existing v8 checkpoint is deliberately not presented as a trained multi-map result: its mean 20-step position error is **7.58px** on the train suite and **3.98px** on the holdout suite, while the two-waypoint task success is **0%** in both splits. The artifact records the per-map values and makes the next gap concrete: train a checkpoint with `--map-suite train`, then repeat the same holdout protocol. See [evaluation-generalization-v8.json](results/evaluation-generalization-v8.json).
