...
The environment now has named map suites rather than only one implicit wall layout. `train` contains `default`, `single_barrier`, and `double_barrier`; `holdout` contains the unseen `cross` and `zigzag` layouts. The new evaluator reports per-map RGB prediction error and a sequential two-waypoint task that must complete two ordered goals. The formal map-suite checkpoint was initialized from v8 and trained for 12 epochs on 1,000 train and 200 validation episodes with 16-step unrolls. At 20 episodes per map and 64 planning candidates, mean 20-step position error is **4.85px** on train maps and **4.84px** on unseen maps; image MAE is **0.0825** versus **0.0915**. Two-waypoint task success is **6.67%** on train maps and **5.00%** on unseen maps, so prediction generalization is now measurable but sequential planning remains unsolved. See [evaluation-map-suite-v1-full.json](results/evaluation-map-suite-v1-full.json).
