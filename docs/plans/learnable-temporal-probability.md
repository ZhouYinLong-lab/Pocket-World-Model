...
The next fallback revision locks the selected route side, compares remaining A* distance with the remaining action budget, and records geometric route progress. It reaches **34.667% ± 6.799pp real success**, **10.733 ± 0.172px** final distance, **2.027 ± 0.349** collisions, and 0.0 emergency side switches under the same protocol. See [locked-budget result](../results/evaluation-route-aware-locked-budget-v8-full.json).

The 48-step budget mismatch was then isolated: the 6px alignment trigger entered fallback too late, leaving about 33 actions for a route that still needed roughly 64px. A 4px trigger recovers **38.83 fallback actions** on average and reaches **50.667% ± 2.494pp real success**, **6.883 ± 0.141px** final distance, **1.140 ± 0.229** collisions, and **7.365px** remaining route distance, with zero emergency side switches. See [alignment-4px result](../results/evaluation-route-aware-alignment4-v8-full.json).
