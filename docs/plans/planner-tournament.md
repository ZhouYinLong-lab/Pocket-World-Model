...
- `cem_collision`: categorical CEM scored with the learned collision head, without explicit wall geometry. The current comparison uses a calibrated-risk budget of 0.15; candidates inside the budget compete on distance, while infeasible candidates fall back to a soft risk penalty.
...
| Collision-aware CEM | 0.0% | 0.0% | 28.71px | 3.55 |
| Route-aware hybrid | — | 100.0% | 2.95px | 0.65 |

The main finding is negative and useful: better action-sequence search does not fix a misspecified obstacle model. CEM and Beam Search both preserve a 100pp imagined–real failure gap on the barrier. Adding the learned collision score to CEM and a 0.15 risk budget changes the distance/collision trade-off, but it does not produce a successful route: collision-aware CEM reaches 0% imagined and 0% real success with 3.55 collisions per episode. Learned collision modeling reduces the gap to 40pp but still fails on a substantial fraction of routes. Closed-loop route-aware control transfers best, at the cost of using explicit visible geometry.
...
- [risk-budget barrier ablation](../results/evaluation-planner-tournament-barrier-v5-risk-budget.json)
